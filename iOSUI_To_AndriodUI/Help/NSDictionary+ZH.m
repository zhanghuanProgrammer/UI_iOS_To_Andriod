        ...
        for (NSString *name in secondObject) {
            ...
            if (object == lastObject) { //当它还只有一个的时候
                ...
                break;
                       ...
                       [object containsObject:lastObject]) { //当它有一群的时候
                nodeName = name;
                break;
            }
        }
...
- (NSArray *)allValuesSortedByKeys {
    ...
    for (id key in sortedKeys) { [arr addObject:self[key]]; }
    return [arr copy];
}
...
+ (NSDictionary *)dictionaryWithXML:(id)xml needRecoderOrder:(BOOL)needRecoderOrder {
    ...
    if ([xml isKindOfClass:[NSString class]]) {
        parser =
        [[_YYXMLDictionaryParser alloc] initWithString:xml needRecoderOrder:needRecoderOrder];
    } else if ([xml isKindOfClass:[NSData class]]) {
        parser =
        [[_YYXMLDictionaryParser alloc] initWithData:xml needRecoderOrder:needRecoderOrder];
    }

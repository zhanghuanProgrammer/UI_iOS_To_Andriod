...
- (NSArray *)removeSameString:(NSArray *)arr {
    ...
    for (NSString *str in arr) {
        if ([arrM containsObject:str] == NO) { [arrM addObject:str]; }
    }
    return arrM;
}
...
- (CompareArray)compareToTextArr:(NSArray *)otherTextArr {
    ...
    NSArray *maxLenArr =
    selfUniqueArr.count > otherUniqueArr.count ? selfUniqueArr : otherUniqueArr;
    NSArray *minLenArr;
    ...
    for (NSString *tempStr in minLenArr) {
        if ([text rangeOfString:[NSString stringWithFormat:@"-%@-", tempStr]].location ==
            NSNotFound) {
            targetArr = minLenArr;
            break;
        }
...
+ (NSMutableArray *)arrayWithPlistData:(NSData *)plist {
    ...
    //    arrayWithContentsOfFile:@""];这个方法里面可能就是调用了下面的这句方法代码来解析的
...
- (void)prependObjects:(NSArray *)objects {
    if (!objects) return;
    NSUInteger i = 0;
    for (id obj in objects) { [self insertObject:obj atIndex:i++]; }
}
/**将子数组插入到父数组的第几个位置*/

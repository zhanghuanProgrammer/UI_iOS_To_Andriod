...
/**
 函数功能描述:为一个树节点赋值
 */
TreeNode *setTree(TreeNode *T, NSString *value, NSString *name, TreeNode *child, TreeNode *brother,
                  propertyNode *propertys) {
    T.brother   = brother;
    T.child     = child;
...
void Post_display(TreeNode *T) {
    //依次从根节点遍历(后序遍历)
    if (T != nil) {
        if (T.child != nil) Post_display(T.child);     //优先打印子节点
        _value(T.value);                               //打印属性
        _name(T.name);                                 //打印名字
        display_propertys(T.propertys);                //依次打印属性的key值和value值
        if (T.brother != nil) Post_display(T.brother); //打印兄弟节点
    }
}
/**
 函数功能描述:层次遍历
...
void TravLevel_display(TreeNode *T) {
    ...
    if (T != nil) {
        ...
        _name(T.name);                                      //打印名字
        display_propertys(T.propertys);                     //依次打印属性的key值和value值
        if (T.brother != nil) TravLevel_display(T.brother); //优先打印兄弟节点
        if (T.child != nil) TravLevel_display(T.child);     //打印子节点
    }
}
...
- (void)displayMyInfo:(NSArray *)arr {
    int i = 0;
    for (NSString *str in arr) { printf("%d  %s\n", ++i, [str UTF8String]); }
}
...
- (NSMutableArray *)split_NoHasFuhao {
    ...
    for (self.cur = 0; self.cur < self.text.length; self.cur++) {
        ...
        if (ch == '<') { [arrM addObject:[self kuohao_NoHasFuhao]]; }
    }
    return arrM;
}
    ...
    for (i = 0; i < [arrM count] - 1; i++) {
        ...
        if ([temp_str2 hasPrefix:@"/"] == NO) {
            ...
            if (range.location != NSNotFound) {
                ...
                temp_str2 = [NSMutableString stringWithString:arrM[i]];
                [temp_str2
                 deleteCharactersInRange:NSMakeRange(range.location + 1, temp_str.length)];
                // temp_str_all=remove_space(temp_str);
                
                temp_str_all = [[temp_str2 stringByAppendingString:@"<key>"]
                                stringByAppendingString:[self remove_space:temp_str]];
                ...
                if ([temp_str_all hasSuffix:@"/"] == YES) {
                    ...
                    [arrM_temp
                     addObject:[@"/" stringByAppendingString:
                                [temp_str_all
                                 substringToIndex:[temp_str_all rangeOfString:@" "]
                                 .location]]];
                    //                 NSLog(@"-------%@",[temp_str_all
                    //                 substringToIndex:[temp_str_all rangeOfString:@"
                    //                 "].location]);
        ...
        if ([[@"/" stringByAppendingString:arrM[i]] hasPrefix:arrM[i + 1]]) {
            ...
            range    = [self.text rangeOfString:temp_str
                                        options:NSLiteralSearch
                                          range:NSMakeRange(now, self.text.length - now)];
        ...
        if (Is_added == NO) { //如果里面有keys
            if ([temp_str_all isEqualToString:@""] == NO) {
                ...
                [arrM_temp addObject:arrM[i]]; //如果里面没有keys，直接保存
        }
        temp_str_all = @""; //重置
    }
...
- (propertyNode *)gets_key:(NSString *)str {
    ...
    //    adjustsFontSizeToFit="NO" translatesAutoresizingMaskIntoConstraints="NO" id="wWT-gU-7Cd">
    ...
    if ([str rangeOfString:@"=\" "].location != NSNotFound) {
        ...
        for (int i = 0; i < [arr count] - 1; i++) {
            ...
            if ([temp1 hasSuffix:@"="] && [temp2 rangeOfString:@"="].location == NSNotFound) {
                [arrM addObject:[[temp1 stringByAppendingString:@"\""]
                                 stringByAppendingString:temp2]];
                i++;
            } else {
    ...
    for (int i = 0; i < [arr count]; i++) {
        ...
        if ([temp rangeOfString:@"="].location == NSNotFound ||
            ...
            NSLog(@"很有可能因为XML格式不对导致错误");
            NSLog(@"出错数据为:%@", temp);
            //这里面是处理第一个兄弟节点的值,所以一般情况下,错误会出在后面连续获取兄弟节点值得过程中,因为那里可能出现
            //属性值有空格的情况
            return nil;
        }
...
- (TreeNode *)next_child_tree:(NSMutableArray *)arr {
    ...
    if (self.arr_cur < [arr count]) {
        ...
        if ([temp hasPrefix:@"/"]) { return nil; } //这句要非常谨慎 代表子节点的结束
        ...
        if ([temp rangeOfString:@"<key>"].location != NSNotFound) { //如果存在keys值
            ...
            if ([temp rangeOfString:@"<value>"].location != NSNotFound) { //如果还存在value值
                ...
                //拿到<key>id = "201502002"
                NSString *keys =
                [temp substringFromIndex:[temp rangeOfString:@"<key>"].location + 5];
                ...
                //获取"李"
                NSString *valueTemp =
                [temp substringFromIndex:[temp rangeOfString:@"<value>"].location + 7];
        ...
        //        display_propertys(P);
        return T_temp = setTree(T_temp, value, name, [self next_child_tree:arr],
                                [self next_brother_tree:arr], P);
    }
    return nil;
}
/**
...
- (TreeNode *)getIdForPath:(TreeNode *)T:(NSArray *)arr:(int)n {
    if (n == [arr count] - 1 && [T.name isEqualToString:arr[n]])
        return T;
    else if (n < [arr count] && (T != nil || T.child != nil)) {
        if ([T.name isEqualToString:arr[n]]) { return [self getIdForPath:T.child:arr:n + 1]; }
        while (T.brother != nil) {
            T = T.brother;
            if ([T.name isEqualToString:arr[n]]) { return [self getIdForPath:T.child:arr:n + 1]; }
        }
...
- (NSString *)attributeForName:(TreeNode *)T:(NSString *)name {
    if (T != nil) {
        if (T.propertys != nil) {
            ...
            if ([P.key isEqualToString:name]) return [P.value copy];
            while (P.next != nil) {
                P = P.next;
                if ([P.key isEqualToString:name]) return [P.value copy];
            }
...
- (void)write_propertysToDict:(NSMutableDictionary *)dicM:(propertyNode *)P {
    if (P != nil) {
        if (P.key != nil && P.value != nil) [dicM setObject:P.value forKey:P.key];
        if (P.next != nil) [self write_propertysToDict:dicM:P.next];
    }
}
- (NSMutableArray *)child_Dic:(TreeNode *)T {
    ...
    if (T != nil) {
        ...
        if (T.value != nil) [dicM setObject:T.value forKey:@"self_Node_value"];
        if (T.propertys != nil) [self write_propertysToDict:dicM:T.propertys];
        if (T.child != nil) {
            ...
            [dicM setObject:@"" forKey:@"child_nodes"];
        [arrM addObject:dicM];
...
- (void)brother_Dic:(TreeNode *)T:(NSMutableArray *)arrM {
    if (T != nil) {
        ...
        if (T.brother != nil) { [self brother_Dic:T.brother:arrM]; }
    }
}
...
- (NSString *)dicNodeName:(NSDictionary *)dic {
    if ([dic[@"self_Node_Name"] isKindOfClass:[NSString class]] &&
        [dic[@"self_Node_Name"] length] > 0) {
...
- (NSString *)dicNodeValueWithKey:(NSString *)key ForDic:(NSDictionary *)dic {
    if ([dic[key] isKindOfClass:[NSString class]] && [dic[key] length] > 0) { return dic[key]; }
    return @"";
}

- (BOOL)checkNodeValue:(id)value {
    if ([value isKindOfClass:[NSString class]] && ([value length] > 0)) { return YES; }
    return NO;
}
...
/**查找整个字典中某个名字的子节点的个数*/
- (NSInteger)countOfTargetNodeWithName:(NSString *)name
...
/**获取指定路径下目标节点的指定key的值*/
- (void)getTargetNodeArrWithName:(NSString *)name
                         withDic:(NSDictionary *)dic
                   withtargetKey:(NSString *)key
                        withArrM:(NSMutableArray *)ArrM {
    if (!ArrM) { ArrM = [NSMutableArray array]; }
...
- (void)getTargetNodeArrWithName:(NSString *)name
                        ...
                        withArrM:(NSMutableArray *)ArrM {
    if (!ArrM) { ArrM = [NSMutableArray array]; }
    if ([[self dicNodeName:dic] isEqualToString:name]) { [ArrM addObject:dic]; }
...
/**获取指定路径下目标节点数组(也就是说,所有目标节点)
 * 查询根据,不是具体的路径,而是某个节点中的属性值*/
- (void)getTargetNodeArrWithKeyName:(NSString *)keyName
    ...
    } else if ([keyValue hasPrefix:@"self.view"] &&
               [self dicNodeValueWithKey:@"id" ForDic:dic].length > 0) {
    ...
    if (childArr != nil) {
        for (id obj in childArr) {
            if ([obj isKindOfClass:[NSDictionary class]]) {
                [self getTargetNodeArrWithKeyName:keyName
                                      andKeyValue:keyValue
                                          withDic:(NSDictionary *)obj
                                         withArrM:ArrM];
            }
        }
    }
...
- (void)getTargetNodeArrWithName:(NSString *)name
    ...
    if (success) {
        ...
        for (NSString *condition in path) {
            if ([self_node_name isEqualToString:condition]) { return; }
        }
    }
    ...
    if (childArr != nil) {
        for (id obj in childArr) {
            if ([obj isKindOfClass:[NSDictionary class]]) {
                ...
                //真的很巧妙
                [self getTargetNodeArrWithName:name
                                       withDic:(NSDictionary *)obj
                                      withArrM:ArrM
                                    notContain:path
                                   withSuccess:success];
    ...
    if (childArr != nil) {
        for (id obj in childArr) {
            if ([obj isKindOfClass:[NSDictionary class]]) {
                ...
                [self getDicFormPathArr:paths withIndex:index withDic:obj];
                if (targetDic != nil) { return targetDic; }
            }
        }
    }
    return nil;
...
/**获取某个路径链表下的所有节点*/
- (NSDictionary *)getDicArrFormPathArr:(NSArray *)paths
                             withIndex:(NSInteger)index
                               withDic:(NSDictionary *)dic
                             addToArrM:(NSMutableArray *)arrM {
...
- (NSDictionary *)getDicWithCondition:(NSDictionary *)condition withDic:(NSDictionary *)dic {
    ...
    for (NSString *str in condition) {
        if (str.length <= 0 || condition[str] == nil) { return nil; }
    }
...
- (NSString *)getPropertyWithName:(NSString *)name
                          withDic:(NSDictionary *)dic
                      needInChild:(BOOL)need {
...
- (NSDictionary *)getOneDegreeChildWithName:(NSString *)name withDic:(NSDictionary *)dic {
    ...
    NSArray *childs = [self childDic:dic];
    for (NSDictionary *subDic in childs) {
        if ([[self dicNodeName:subDic] isEqualToString:name]) { return subDic; }
    }
    return nil;
}

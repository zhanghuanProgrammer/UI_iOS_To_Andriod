...
+ (FileType)getFileType:(NSString *)filePath {
    ...
    if (fileAttributes != nil) {
        ...
        } else if ([[fileAttributes objectForKey:NSFileType]
...
+ (NSString *)getFileNameFromFilePath:(NSString *)filePath {
    return
    [filePath stringByReplacingOccurrencesOfString:[[filePath stringByDeletingLastPathComponent]
...
+ (NSString *)getFilePathRemoveFileName:(NSString *)filePath {
    ...
    if ([filePath hasSuffix:@"/"]) { filePath = [filePath substringToIndex:filePath.length - 1]; }
    return filePath;
}
...
+ (NSString *)getMacHomeDirectorInIOS {
    if ([NSHomeDirectory() rangeOfString:@"Library/Developer"].location != NSNotFound) {
        NSString *path = [NSHomeDirectory()
                          substringToIndex:[NSHomeDirectory() rangeOfString:@"Library/Developer"].location];
...
+ (NSString *)getUpLeverDirector:(NSString *)DirectorPath {
    ...
    if (![director isEqualToString:@"/"]) { return director; }
    return @"";
}
...
+ (NSString *)creatRandomDirectorContainDigitalInDirector:(NSString *)DirectorPath {
    if (![[NSFileManager defaultManager] fileExistsAtPath:DirectorPath]) { return @""; }
    NSString *fileName = [self getDigitalFileName];
    while ([[NSFileManager defaultManager]
            fileExistsAtPath:[DirectorPath stringByAppendingPathComponent:fileName]]) {
        fileName = [self getDigitalFileName];
    }
    [[NSFileManager defaultManager]
     createDirectoryAtPath:[DirectorPath stringByAppendingPathComponent:fileName]
     withIntermediateDirectories:YES
     attributes:nil
     error:nil];
...
+ (NSString *)creatRandomFileContainDigitalInDirector:(NSString *)DirectorPath
    ...
    [[NSFileManager defaultManager]
     createFileAtPath:[DirectorPath stringByAppendingPathComponent:fileName]
     contents:nil
     attributes:nil];
    return [DirectorPath stringByAppendingPathComponent:fileName];
}
+ (NSString *)creatRandomFileContainCharacterInDirector:(NSString *)DirectorPath
                                          pathExtension:(NSString *)pathExtension {
    if (![[NSFileManager defaultManager] fileExistsAtPath:DirectorPath]) { return @""; }
...
+ (NSString *)creatRandomFileContainDigitalCharacterInDirector:(NSString *)DirectorPath
                                                 pathExtension:(NSString *)pathExtension {
    if (![[NSFileManager defaultManager] fileExistsAtPath:DirectorPath]) { return @""; }
...
+ (void)deleteDirectorIfExsit:(NSString *)DirectorPath {
    if ([self fileExistsAtPath:DirectorPath]) [ZHFileManager removeItemAtPath:DirectorPath];
}
/**清空文件夹,如果它存在*/
...
+ (void)deleteFiles:(NSArray *)files {
    for (NSString *file in files) {
        if ([self fileExistsAtPath:file]) [ZHFileManager removeItemAtPath:file];
    }
}
/**删除子路径 文件(带特定后缀) */
+ (void)deleteSubPathFileArrInDirector:(NSString *)DirectorPath
                      hasPathExtension:(NSArray *)pathExtension {
    NSArray *files = [self subPathFileArrInDirector:DirectorPath hasPathExtension:pathExtension];
    [self deleteFiles:files];
}
/**删除子路径 文件(文件名包含指定字符串) */
+ (void)deleteSubPathFileArrInDirector:(NSString *)DirectorPath
                       fileNameContain:(NSArray *)contents {
...
+ (NSArray *)subPathArrInDirector:(NSString *)DirectorPath {
    if (![[NSFileManager defaultManager] fileExistsAtPath:DirectorPath]) { return nil; }
    ...
    if (pathArr.count > 0) { return pathArr; }
    return nil;
}
+ (NSArray *)subPathFileArrInDirector:(NSString *)DirectorPath {
    ...
    for (NSString *str in arrTemp) {
        if (str.length > 0) {
            if ([self getFileType:[DirectorPath stringByAppendingPathComponent:str]] ==
                FileTypeFile) {
                [pathFileArr addObject:[DirectorPath stringByAppendingPathComponent:str]];
            }
        }
    }
    if (pathFileArr.count > 0) { return pathFileArr; }
    return nil;
}
+ (NSArray *)subPathDirectorArrInDirector:(NSString *)DirectorPath {
    ...
    if (pathDirectorArr.count > 0) { return pathDirectorArr; }
    return nil;
}
+ (NSArray *)contentsDirectorArrInDirector:(NSString *)DirectorPath {
    if (![[NSFileManager defaultManager] fileExistsAtPath:DirectorPath]) { return nil; }
    NSArray *arrTemp =
    [[NSFileManager defaultManager] contentsOfDirectoryAtPath:DirectorPath error:nil];
    NSMutableArray *pathDirectorArr = [NSMutableArray array];
    for (NSString *str in arrTemp) {
        if (str.length > 0) {
            if ([self getFileType:[DirectorPath stringByAppendingPathComponent:str]] ==
...
+ (NSArray *)subPathFileArrInDirector:(NSString *)DirectorPath
                  pathContainDirector:(NSArray *)Directors {
    NSArray *fileArr            = [self subPathFileArrInDirector:DirectorPath];
    NSMutableArray *pathFileArr = [NSMutableArray array];
    for (NSString *filePath in fileArr) {
        ...
        for (NSString *subLever in [fileNameNoFileName componentsSeparatedByString:@"/"]) {
            if (subLever.length > 0) { [leverArr addObject:subLever]; }
        }
...
+ (NSArray *)subPathFileArrInDirector:(NSString *)DirectorPath
          pathLessLimiteLevelDirector:(NSInteger)Level {
    NSArray *fileArr            = [self subPathFileArrInDirector:DirectorPath];
    NSMutableArray *pathFileArr = [NSMutableArray array];
    for (NSString *filePath in fileArr) {
        ...
        if (leverArr.count <= Level) { [pathFileArr addObject:filePath]; }
    }
    if (pathFileArr.count > 0) { return pathFileArr; }
    return nil;
}
+ (NSArray *)subPathFileArrInDirector:(NSString *)DirectorPath
          pathMoreLimiteLevelDirector:(NSInteger)Level {
    NSArray *fileArr            = [self subPathFileArrInDirector:DirectorPath];
    NSMutableArray *pathFileArr = [NSMutableArray array];
    for (NSString *filePath in fileArr) {
...
+ (BOOL)arr:(NSArray *)arr HasContainObjectInOtherArr:(NSArray *)otherArr {
    for (NSString *str in arr) {
        for (NSString *subStr in otherArr) {
            if ([str isEqualToString:subStr]) { return YES; }
        }
    }
    return NO;
}
